...
### 5. MeDev (DevProfile) — Single Source of Truth для разработчиков
*Статус: MVP завершен (Frontend + Backend, OAuth2, AI Groq SSE, Drag-and-Drop)*
*Стек: React, TypeScript (dnd-kit), Java (Spring Boot), OAuth2, AI Groq (LLM), Thymeleaf + Flying Saucer (PDF)*

Платформа, решающая проблему синхронизации резюме и GitHub-профиля. MeDev собирает данные о разработчике из разных источников, автоматически парсит неструктурированные PDF-резюме через ИИ и генерирует на их основе как красивую веб-страницу (портфолио), так и готовый PDF-документ.

**Что сделано:**
- Реализован Data-First подход, где данные из GitHub (репозитории, коммиты, PR) синхронизируются через API.
- Встроен AI-модуль для интеллектуального парсинга существующих резюме пользователя в единую структуру данных.
- Визуальный конструктор портфолио с Drag-and-Drop на фронтенде (`dnd-kit`).
- Генерация PDF-документов на сервере из HTML/CSS шаблонов.

**Что понял на этом проекте:**
- Как интегрировать LLM для решения практических задач парсинга неструктурированных данных.
- Опыт работы с Server-Sent Events (SSE) для стриминга ответов ИИ.

### 6. Air Canvas — рисование жестами в воздухе
*Статус: экспериментальный AI-проект*
*Стек: React, HTML5 Canvas, Python (FastAPI), OpenCV, MediaPipe, WebSockets*

Интерактивное веб-приложение нового поколения (Natural User Interface), позволяющее пользователю рисовать на экране компьютера, просто перемещая палец в воздухе перед веб-камерой. Приложение захватывает видеопоток, анализирует положение кисти руки в пространстве, распознает жесты и транслирует их в команды рисования на HTML5 Canvas.

**Что сделано:**
- Real-time трекинг руки: Точное отслеживание 21 ключевой точки кисти с частотой до 30 FPS через MediaPipe.
- Распознавание жестов на лету (Рисование, Пауза, Очистка).
- Двухсторонняя связь через WebSockets между Python-сервисом (компьютерное зрение) и React-фронтендом (рендеринг).

**Что понял на этом проекте:**
- Как связывать тяжелые вычисления на Python с быстрыми интерфейсами на React в реальном времени.
- Работа с потоковыми данными и оптимизация рендеринга на Canvas для плавной отрисовки.

### 7. Second Brain (База Знаний)
*Статус: активно пополняется*
*Стек: Markdown, Zettelkasten, Obsidian*

Мой личный [архитектурный репозиторий](https://github.com/MrSgemaSeny/Second-Brain), структурированный по методологии Zettelkasten. Это не просто заметки, а детальные разборы архитектурных паттернов, лог принятых решений (ADR) и документация ко всем моим проектам.

Здесь собраны глубокие исследования тем: от тюнинга JVM Metaspace и паттернов проектирования Telegram-ботов до архитектуры кэширования Caffeine и реализации Multi-Tenancy. Наличие Second Brain отражает мой подход к разработке: сначала анализ и проектирование, затем — код.
